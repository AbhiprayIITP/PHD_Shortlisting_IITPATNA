...
def WrtFile(row , index):                                                      #Function for writing selected students to xlsx file
...
for ind in df_in.index:
    ...
    if(o.match(IIT_check)!= None or p.search(IIT_check)!=None ):
        IISC_flag = 1         #Checking if IISC 
    else:
        IISC_flag = 0
    ...
    if(Btech_check or MCA_check or MSC_check):
         ugcpi_l,ugper_l,imper_l,hscpi_l,hsper_l = 6.5,60,60,6.5,60
         if(qcpi>=8.5 or qper>=80 or (Btech_check and IIT_flag and qcpi>=8.0)):
             if(imper>=60):
                 hscpi_l = 6
                 hsper_l = 55
             elif(hscpi >= 6.5 or hsper >= 60):
    ...
    if((Mtech_check or MS_check or ME_check) and disp_flag and (qcpi>=6.5 or qper>=60) and (ugcpi>=ugcpi_l or ugper>=ugper_l) and (imper>=imper_l) and (hscpi>= hscpi_l or hsper>=hsper_l) and (gate == 'Yes')):
        if((category=='General' and age<=32) or (category=='OBC Non Creamy Layer' and age<=37) or (category== 'EWS' and age<=37) or (gender == 'Female' and age<=37)):
            WrtFile(row,ind)
            row = row+1
    elif((Btech_check or MCA_check or MSC_check) and disp_flag and (qcpi>=8 or qper>=75)  and (ugcpi>=ugcpi_l or ugper>=ugper_l) and (imper>=imper_l) and (hscpi>= hscpi_l or hsper>=hsper_l) and (((gate=='Yes') and (gate_rank<=5000)) or Gate_exemp)):
        if((category=='General' and age<=28) or (category=='OBC Non Creamy Layer' and age<=33) or (category=='EWS' and age<=33) or (gender == 'Female' and age<=33)):
            WrtFile(row,ind)
            row = row+1
            
    if((category == 'SC' or category == 'ST' or pd == 'Yes')):
        if((Mtech_check or MS_check or ME_check) and disp_flag and (qcpi>=6 or qper>=55) and (ugcpi>=ugcpi_l or ugper>=ugper_l) and (imper>=imper_l) and (hscpi>= hscpi_l or hsper>=hsper_l) and (gate == 'Yes') and age<=37):
            WrtFile(row,ind)
            row = row+1
        elif((Btech_check or MCA_check or MSC_check) and disp_flag and (qcpi>=8 or qper>=75)  and (ugcpi>=ugcpi_l or ugper>=ugper_l) and (imper>=imper_l) and  (hscpi>= hscpi_l or hsper>=hsper_l) and (((gate=='Yes') and (gate_rank<=6000)) or Gate_exemp) and age<= 33):
            WrtFile(row,ind)
            row = row+1
